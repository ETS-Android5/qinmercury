...
public class InAppChannel extends InAppBase {
	...
	//comment
	private String Channelname="InAppXiaomi";
	private String app_id="2882303761517402635";
	private String app_key="5151740219635";
	...
	{		
		...
		HyDJ.getInstance().onMainActivityCreate(mContext);

	}
	public void ApplicationInit(Application appcontext)
	{
		mAppContext=appcontext;
		HyDJ.init(appcontext, app_id, app_key, new InitCallback() {
			@Override
			public void onInitCompleted() {
				MercuryActivity.LogLocal("["+Channelname+"]->init:InAppChannel.ApplicationInit=");
				// after init completed, you can enter the game.
			}
...
//		TestPay();
		...
		HyDJ.getInstance().wxPay(mContext, purchase, new PayResultCallback() {
			@Override
	...
	{
		HyDJ.getInstance().onTerminate((Application) mAppContext);
	}
	//end
}
